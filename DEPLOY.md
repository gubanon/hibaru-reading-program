...
## Email: Gmail SMTP (optional, but needed for invite/notification emails)

Classroom-invite "Join" links and new-assignment notifications are sent via
Nodemailer through a Gmail account's SMTP server. Without this set up, the
app still works fully — invites and notifications are just skipped (an
invite still creates a working `/join/:token` link, it just isn't emailed
automatically; share it manually if needed).

1. Use an existing Gmail account, or create a new one dedicated to the app
   (e.g. `hibaru.notify@gmail.com`) — either works.
2. Go to **myaccount.google.com/security** → turn on **2-Step
   Verification** if it isn't already on (required for the next step).
3. Search settings for **App passwords** → create one, pick "Mail" as the
   app → copy the generated 16-character password. This is NOT the same as
   the account's normal login password.
4. Treat that app password like a secret — don't paste it into chat, only
   into Render's environment variable field (or your local `.env`).
...
3. It will prompt for the env vars marked `sync: false` in `render.yaml` —
   ...
   - `EMAIL_USER` — the Gmail address from the Email section above (optional —
     leave blank to skip email sending)
   - `EMAIL_APP_PASSWORD` — the 16-character app password from that same
     section (optional, same as above)
