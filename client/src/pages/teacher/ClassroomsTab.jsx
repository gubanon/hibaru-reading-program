...
const inputSm = { fontFamily: "inherit", fontSize: 12, padding: "7px 9px", border: "1px solid var(--input-border)", borderRadius: 7, outline: "none", background: "var(--card-bg)" };
    ...
    <div style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)", borderRadius: 14, padding: 20 }}>
      ...
      <div style={{ marginTop: 14, padding: 12, background: "var(--subtle-bg)", borderRadius: 10 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: ".6fr 1fr 1.4fr", gap: 6, marginTop: 6 }}>
          ...
          <input style={inputSm} value={inv.email} onChange={e => setInv({ ...inv, email: e.target.value })} placeholder="student@example.com" />
        </div>
        {err && <div style={{ color: "#B3261E", fontSize: 11.5, marginTop: 6 }}>{err}</div>}
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
          <div style={{ flex: 1, fontSize: 10.5, color: "var(--text-faint-2)" }}>School, Division &amp; Region auto-fill: Taft NHS (303529) · Eastern Samar · Region VIII</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(400px,1fr))", gap: 16 }}>
        ...
        <div style={{ border: "2px dashed #DDDACE", borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", gap: 10, justifyContent: "center" }}>
          ...
          <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. Grade 8 – Bonifacio"
            style={{ fontFamily: "inherit", fontSize: 13, padding: "10px 12px", border: "1px solid var(--input-border)", borderRadius: 8, outline: "none" }} />
