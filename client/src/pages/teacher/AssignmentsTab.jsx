...
import { api } from "../../api";
import { FAINT, NAVY, GREEN } from "../../theme";
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20, marginTop: 12 }}>
        {groups.map(g => (
          <div key={g.name}>
            <div style={{ fontSize: 12, fontWeight: 700, color: NAVY, letterSpacing: ".06em", marginBottom: 8 }}>{g.name}</div>
            <div style={{ background: "#fff", border: "1px solid #E7E5DD", borderRadius: 14, overflow: "hidden" }}>
              {g.items.map(a => (
                <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "15px 18px", borderBottom: "1px solid #F4F2EC" }}>
                  <div style={{ width: 40, height: 40, borderRadius: 10, background: "#E9EDF7", display: "grid", placeItems: "center", fontSize: 19 }}>📄</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <button onClick={() => onEdit(a.id)} style={{ border: "none", background: "none", cursor: "pointer", padding: 0, fontFamily: "inherit", fontSize: 15, fontWeight: 700, color: NAVY, textAlign: "left" }}>{a.title}</button>
                    <div style={{ fontSize: 12, color: FAINT, marginTop: 2 }}>{a.dateText} · {a.meta}</div>
                    <div style={{ fontSize: 12, fontWeight: 600, marginTop: 3, color: a.complete ? GREEN : a.partial ? "#B8860B" : "#A9A89E" }}>✓ {a.doneText}</div>
                  </div>
                  <div style={{ display: "flex", gap: 7 }}>
                    <button onClick={() => onView(a.id)} style={{ border: "1px solid #E0DED5", cursor: "pointer", padding: "7px 13px", borderRadius: 8, background: "#fff", fontFamily: "inherit", fontSize: 12, fontWeight: 600 }}>👁 View</button>
                    <button onClick={() => onPrint(a.id)} style={{ border: "1px solid #E0DED5", cursor: "pointer", padding: "7px 13px", borderRadius: 8, background: "#fff", fontFamily: "inherit", fontSize: 12, fontWeight: 600 }}>🖨 Print</button>
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <button onClick={onBack} style={{ border: "1px solid #E0DED5", cursor: "pointer", padding: "8px 14px", borderRadius: 8, background: "#fff", fontFamily: "inherit", fontSize: 12.5, fontWeight: 600 }}>← Cancel</button>
        ...
        <div style={{ background: "#fff", border: "1px solid #E7E5DD", borderRadius: 14, padding: 22, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 5 }}>
              ...
              <div style={{ fontSize: 11.5, color: NAVY, fontWeight: 600 }}>{wordCount(a.passage)} words</div>
            </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: 8 }}>
              {questionTexts.map((q, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, padding: "7px 10px", background: "#FAF9F5", borderRadius: 7, color: "#4A4940" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={addQ} style={{ border: "none", cursor: "pointer", padding: "9px 16px", borderRadius: 8, background: "#F0EEE7", color: "#26251F", fontFamily: "inherit", fontSize: 13, fontWeight: 600 }}>Add</button>
            </div>
  ...
  return (
    <>
      <div data-noprint="1" style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        ...
        <div style={{ flex: 1 }} />
        <button onClick={() => api.download(`/teacher/assignments/${id}/docx`, `Assignment - ${a.title}.docx`)} style={{ border: "1px solid #E0DED5", cursor: "pointer", padding: "8px 14px", borderRadius: 8, background: "#fff", fontFamily: "inherit", fontSize: 12.5, fontWeight: 600 }}>⬇ DOCX</button>
        <button onClick={() => window.print()} style={{ border: "none", cursor: "pointer", padding: "8px 16px", borderRadius: 8, background: "#26251F", color: "#fff", fontFamily: "inherit", fontSize: 12.5, fontWeight: 600 }}>🖨 Print</button>
      </div>
      <div data-print-area="1" style={{ background: "#fff", border: "1px solid #E7E5DD", borderRadius: 14, padding: 32, maxWidth: 780, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, borderBottom: "2px solid #22335E", paddingBottom: 12, marginBottom: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#8A897F", letterSpacing: ".08em" }}>READING ASSIGNMENT · {a.className}</div>
            <h2 style={{ margin: "4px 0 0", fontSize: 21, fontWeight: 700 }}>{a.title}</h2>
          </div>
          <img src="/assets/taft-logo.png" alt="" style={{ height: 44 }} />
        </div>
        <div style={{ fontSize: 12.5, color: "#6B6A63", marginBottom: 12 }}>{a.genre} · {a.wordCount} words · Attempts: {a.attempts} · {a.timeLimit} · Sensitivity: {a.sensitivity} · Due {a.deadlineISO}</div>
        <div style={{ fontSize: 13.5, fontStyle: "italic", color: "#4A4940", padding: "12px 14px", background: "#FAF9F5", borderRadius: 10, marginBottom: 18 }}>{a.instructions}</div>
          ...
          {a.questions.map((q, i) => (
            <div key={i}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 18px", marginLeft: 16 }}>
                {q.options.map((o, oi) => (
                  <div key={oi} style={{ fontSize: 13, color: "#4A4940" }}><b style={{ color: "#22335E" }}>{["A", "B", "C", "D"][oi]}.</b> {o}</div>
                ))}
              </div>
            </div>
          ))}
