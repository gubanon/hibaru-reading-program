...
import { api } from "../../api";
import { FAINT, NAVY, GREEN, MISCUE_TYPES, levelColor, statusMeta } from "../../theme";
  ...
  return (
    <>
      <div data-noprint="1" style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <button onClick={onBack} style={{ border: "1px solid #E0DED5", cursor: "pointer", padding: "8px 14px", borderRadius: 8, background: "#fff", fontFamily: "inherit", fontSize: 12.5, fontWeight: 600 }}>← Back</button>
        <div style={{ flex: 1 }} />
        <button onClick={() => setShowParent(true)} style={{ border: "1px solid #E0DED5", cursor: "pointer", padding: "8px 14px", borderRadius: 8, background: "#fff", fontFamily: "inherit", fontSize: 12.5, fontWeight: 600 }}>👨‍👩‍👧 Parent summary</button>
        <button onClick={() => window.print()} style={{ border: "none", cursor: "pointer", padding: "8px 16px", borderRadius: 8, background: "#26251F", color: "#fff", fontFamily: "inherit", fontSize: 12.5, fontWeight: 600 }}>🖨 Print Phil-IRI Form 3</button>
      </div>
      <div data-print-area="1" style={{ background: "#fff", border: "1px solid #E7E5DD", borderRadius: 14, padding: 26 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, borderBottom: "2px solid #22335E", paddingBottom: 12, marginBottom: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#8A897F", letterSpacing: ".08em" }}>PHIL-IRI FOR JHS FORM 3 · LEARNER'S RECORD</div>
            <h2 style={{ margin: "4px 0 0", fontSize: 22, fontWeight: 700 }}>{r.name}</h2>
          </div>
          <div style={{ marginLeft: "auto", textAlign: "right", fontSize: 12, color: "#6B6A63", lineHeight: 1.6 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 12, marginBottom: 22 }}>
          {[
            { label: "READING RATE", value: m.wpm + " wpm", sub: `${Math.floor(r.seconds / 60)}:${String(r.seconds % 60).padStart(2, "0")} total (${r.seconds}s)`, color: "#26251F" },
            ...
            <div key={i} style={{ background: "#FAF9F5", borderRadius: 11, padding: "14px 16px" }}>
              <div style={{ fontSize: 10.5, fontWeight: 700, color: "#8A897F", letterSpacing: ".05em" }}>{st.label}</div>
              <div style={{ fontSize: 22, fontWeight: 700, marginTop: 3, color: st.color }}>{st.value}</div>
              <div style={{ fontSize: 11, color: "#8A897F", marginTop: 1 }}>{st.sub}</div>
            </div>
          ))}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 22, alignItems: "start" }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 10 }}>Part B · Miscue Analysis <span style={{ color: "#8A897F", fontWeight: 500 }}>· {m.tm} total</span></div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {bars.map(b => (
                <div key={b.key} style={{ display: "grid", gridTemplateColumns: "150px 1fr 22px", gap: 8, alignItems: "center", fontSize: 12 }}>
                  <div style={{ color: "#4A4940" }}>{b.label} <span style={{ color: "#A9A89E", fontStyle: "italic" }}>{b.fil}</span></div>
                  <div style={{ height: 14, background: "#F0EEE7", borderRadius: 999, overflow: "hidden" }}><div style={{ height: "100%", borderRadius: 999, background: b.color, width: `${b.pct}%` }} /></div>
                  <div style={{ fontWeight: 700, textAlign: "right" }}>{b.count}</div>
                </div>
              ))}
            </div>
            <div style={{ marginTop: 18, padding: 14, background: "#FAF9F5", borderRadius: 11, fontSize: 12.5, lineHeight: 1.6, color: "#4A4940" }}>
            ...
            <div style={{ marginTop: 12, padding: 14, border: `2px solid ${rProfileColor}`, borderRadius: 11 }}>
              <div style={{ fontSize: 10.5, fontWeight: 700, color: "#8A897F", letterSpacing: ".06em" }}>PART C · READING PROFILE (per passage)</div>
              <div style={{ fontSize: 21, fontWeight: 700, color: rProfileColor, marginTop: 3 }}>{m.profile}</div>
              <div style={{ fontSize: 11.5, color: "#8A897F", marginTop: 2 }}>Word Reading {m.level} × Comprehension {m.compLevel}</div>
            </div>
          </div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 10 }}>Marked-up passage <span style={{ color: "#8A897F", fontWeight: 500 }}>· miscues shown in context</span></div>
            <div style={{ border: "1px solid #EFEDE6", borderRadius: 11, padding: 18, fontSize: 15, lineHeight: 2.1 }}>
              {r.marked.map((w, i) => {
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 10 }}>
              {legend.map(l => (
                <span key={l.key} style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11.5, color: "#6B6A63" }}>
                  <span style={{ width: 11, height: 11, borderRadius: 3, background: l.color }} />{l.label}
                </span>
              ))}
              {!legend.length && <span style={{ fontSize: 11.5, color: "#8A897F" }}>No miscues detected.</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      {showParent && (
        <div data-noprint="1" onClick={() => setShowParent(false)} style={{ position: "fixed", inset: 0, background: "rgba(34,51,94,.5)", display: "grid", placeItems: "center", zIndex: 100, padding: 24 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, padding: 28, maxWidth: 520, fontSize: 14, lineHeight: 1.7 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#8A897F", letterSpacing: ".08em", marginBottom: 6 }}>SUMMARY FOR PARENT / GUARDIAN · PARA SA MAGULANG</div>
...
function PrintAllSheet({ assignmentId, onBack }) {
  ...
  return (
    <>
      <div data-noprint="1" style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <button onClick={onBack} style={{ border: "1px solid #E0DED5", cursor: "pointer", padding: "8px 14px", borderRadius: 8, background: "#fff", fontFamily: "inherit", fontSize: 12.5, fontWeight: 600 }}>← Back</button>
        <div style={{ fontSize: 13, color: "#6B6A63" }}>Print-ready · one Phil-IRI Form 3 Learner's Record per page</div>
        <div style={{ flex: 1 }} />
        <button onClick={() => api.download(`/teacher/assignments/${assignmentId}/reports-all.docx`, "Phil-IRI Form 3.docx")} style={{ border: "1px solid #E0DED5", cursor: "pointer", padding: "8px 14px", borderRadius: 8, background: "#fff", fontFamily: "inherit", fontSize: 12.5, fontWeight: 600 }}>⬇ DOCX</button>
        <button onClick={() => window.print()} style={{ border: "none", cursor: "pointer", padding: "8px 16px", borderRadius: 8, background: "#26251F", color: "#fff", fontFamily: "inherit", fontSize: 12.5, fontWeight: 600 }}>🖨 Print / Save as PDF</button>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        {records.map((rec, i) => {
          const m = rec.metrics;
          return (
            <div key={i} data-record="1" data-print-area="1" style={{ background: "#fff", border: "1px solid #E7E5DD", borderRadius: 14, padding: "44px 52px", fontFamily: "'Times New Roman',Georgia,serif", fontSize: 14, color: "#000", maxWidth: 760, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
    ...
    <>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: "#6B6A63", marginTop: 4 }}>{progress.title} · Due {progress.deadline} · {progress.completionPct}% turned in</div>
        </div>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ background: "#fff", border: "1px solid #E7E5DD", borderRadius: 14, overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1.2fr .8fr .9fr 1fr 1.1fr 100px", gap: 8, padding: "12px 18px", fontSize: 11.5, fontWeight: 700, color: "#8A897F", letterSpacing: ".04em", borderBottom: "1px solid #EFEDE6" }}>
          <div>STUDENT</div><div>STATUS</div><div>WPM</div><div>WORD SCORE</div><div>COMPREHENSION</div><div>READING PROFILE</div><div></div>
        </div>
        {progress.rows.map(r => {
          const sm = statusMeta[r.status];
          return (
            <div key={r.submissionId} style={{ display: "grid", gridTemplateColumns: "2fr 1.2fr .8fr .9fr 1fr 1.1fr 100px", gap: 8, padding: "13px 18px", alignItems: "center", borderBottom: "1px solid #F4F2EC", fontSize: 13.5 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "#A9A89E" }}>{r.grade}</div>
                </div>
              </div>
              ...
              <div style={{ fontWeight: 600, color: r.profile ? levelColor(r.profile) : "#A9A89E" }}>{r.profile || "—"}</div>
              <div>{r.hasReport && <button onClick={() => setReportId(r.submissionId)} style={{ border: "1px solid #E0DED5", cursor: "pointer", padding: "6px 12px", borderRadius: 8, background: "#fff", fontFamily: "inherit", fontSize: 12, fontWeight: 600, color: NAVY }}>Report →</button>}</div>
            </div>
          );
        })}
      </div>
