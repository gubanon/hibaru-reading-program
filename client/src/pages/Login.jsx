...
export default function Login() {
  ...
  const termsCheckbox = (
    <label style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 18, fontSize: 12, color: "#6B6A63", cursor: "pointer" }}>
      <input type="checkbox" checked={agreeTerms} onChange={e => setAgreeTerms(e.target.checked)} style={{ marginTop: 2 }} />
      <span>I agree to the <a href="/terms" target="_blank" rel="noreferrer" style={{ color: NAVY, fontWeight: 600 }}>Terms &amp; Conditions</a> and <a href="/privacy" target="_blank" rel="noreferrer" style={{ color: NAVY, fontWeight: 600 }}>Privacy Policy</a>.</span>
    </label>
  );
  ...
  function cardStyle(r) {
    return { border: role === r ? NAVY : "#E0DED5", bg: role === r ? "#E9EDF7" : "#fff" };
  }

  return (
    <div style={{ minHeight: "100vh", display: "grid", gridTemplateColumns: "1.1fr 1fr", background: NAVY }}>
      <div style={{ position: "relative", display: "flex", flexDirection: "column", justifyContent: "center", padding: "64px 72px", color: "#fff", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 36 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#fff", borderRadius: 16, padding: "10px 14px" }}>
      ...
      <div style={{ background: "#F6F5F1", borderRadius: "28px 0 0 28px", display: "flex", alignItems: "center", justifyContent: "center", padding: 48 }}>
        <div style={{ width: "100%", maxWidth: 400 }}>
          {mode === "signin" && (
            <>
              <h2 style={{ margin: "0 0 6px", fontSize: 26, fontWeight: 700 }}>Welcome back</h2>
              <div style={{ fontSize: 13.5, color: "#6B6A63", marginBottom: 26 }}>{welcomeMap[role]}</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 22 }}>
                <button onClick={() => setRole("teacher")} style={{ border: `2px solid ${cardStyle("teacher").border}`, cursor: "pointer", borderRadius: 14, padding: "16px 12px", background: cardStyle("teacher").bg, fontFamily: "inherit", textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 700, marginTop: 6, color: "#26251F" }}>Teacher</div>
                  <div style={{ fontSize: 11, color: "#8A897F", marginTop: 2 }}>Guro</div>
                </button>
                <button onClick={() => setRole("student")} style={{ border: `2px solid ${cardStyle("student").border}`, cursor: "pointer", borderRadius: 14, padding: "16px 12px", background: cardStyle("student").bg, fontFamily: "inherit", textAlign: "center" }}>
                  <div style={{ fontSize: 26 }}>🧑‍🎓</div>
                  <div style={{ fontSize: 14, fontWeight: 700, marginTop: 6, color: "#26251F" }}>Student</div>
                  <div style={{ fontSize: 11, color: "#8A897F", marginTop: 2 }}>Mag-aaral</div>
                </button>
              </div>
              ...
              {role === "teacher" && (
                <>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "20px 0 14px" }}>
                    <div style={{ flex: 1, height: 1, background: "#E0DED5" }} />
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#A9A89E", letterSpacing: ".06em" }}>NEW TEACHER?</div>
                    <div style={{ flex: 1, height: 1, background: "#E0DED5" }} />
                  </div>
                  <button onClick={() => { setMode("signup"); setSignupDone(false); setError(""); setAgreeTerms(false); }} style={{ width: "100%", border: `1.5px solid ${NAVY}`, cursor: "pointer", padding: 13, borderRadius: 12, background: "transparent", color: NAVY, fontFamily: "inherit", fontSize: 14, fontWeight: 700 }}>Create a teacher account</button>
                </>
              )}
              <div style={{ textAlign: "center", marginTop: 16, fontSize: 12, color: "#A9A89E" }}>Invited students: sign in with your DepEd email — you'll be asked to set a password the first time.</div>
              <div style={{ textAlign: "center", marginTop: 10, fontSize: 11.5, color: "#A9A89E" }}>
                <a href="/terms" target="_blank" rel="noreferrer" style={{ color: "#8A897F" }}>Terms &amp; Conditions</a>
                {" · "}
                <a href="/privacy" target="_blank" rel="noreferrer" style={{ color: "#8A897F" }}>Privacy Policy</a>
              </div>
            </>
          )}
          ...
          {mode === "claim" && (
            <>
              ...
              <button onClick={() => { setMode("signin"); setError(""); setAgreeTerms(false); }} style={{ width: "100%", marginTop: 14, border: "none", background: "none", cursor: "pointer", padding: 8, fontFamily: "inherit", fontSize: 13, fontWeight: 600, color: NAVY }}>← Back to sign in</button>
            </>
          )}
            ...
            <>
              {!signupDone ? (
                ...
                <div style={{ textAlign: "center", padding: "20px 0" }}>
                  ...
                  <div style={{ fontSize: 13.5, color: "#6B6A63", lineHeight: 1.7, maxWidth: 320, margin: "0 auto" }}>Your teacher account is now <b>pending admin approval</b>. You'll receive an email at your DepEd address once it's activated.</div>
                </div>
              )}
              <button onClick={() => { setMode("signin"); setError(""); setAgreeTerms(false); }} style={{ width: "100%", marginTop: 14, border: "none", background: "none", cursor: "pointer", padding: 8, fontFamily: "inherit", fontSize: 13, fontWeight: 600, color: NAVY }}>← Back to sign in</button>
            </>
            ...
            <>
              {!fpDone ? (
                ...
                <div style={{ textAlign: "center", padding: "20px 0" }}>
                  ...
                  <div style={{ fontSize: 13.5, color: "#6B6A63", lineHeight: 1.7, maxWidth: 320, margin: "0 auto" }}>If an account exists for that email, a password reset link is on its way.</div>
                </div>
              )}
              <button onClick={() => { setMode("signin"); setError(""); }} style={{ width: "100%", marginTop: 14, border: "none", background: "none", cursor: "pointer", padding: 8, fontFamily: "inherit", fontSize: 13, fontWeight: 600, color: NAVY }}>← Back to sign in</button>
            </>
          )}
        </div>
      </div>
